...
An IDX, which stands for Internet Data Exchange, is a data feed that allows real estate websites to display MLS listings. This data includes information on homes for sale and recent sales.

By using an IDX on your website, you can give your visitors access to this valuable information. So, if you want to add MLS to your Webflow website, you'll need to add an IDX feed.
...
Within two business days, IDX Broker should contact you with your account information (login/password) and some additional steps to complete the link between your IDX Broker account and your local MLS data provider.
...
Now that your account is all setup, it's time to configure it.
...
After choosing your domain, you'll receive some DNS records. In your domain's control panel, create some DNS records with the host as your selected name (in our case, "search") and everything else described by IDX Broker. This is the same process you go through when setting up a custom domain on your Webflow site.

Save your DNS changes and wait up to 24-48 hours for them to propagate. It usually takes a lot less, but be prepared for a long wait.
...
Still on the Wrappers Management page, select the "Global" tab, and under Dynamic URL enter your site's published domain followed by **/idx-wrapper**.
...
Go to your Webflow site and create a new page called IDX Wrapper. Make sure the slug matches the slug you entered into IDX Broker for your global wrapper. If you're following this guide to a tee, your slug should be /idx-header.
...
Inside your container component, you can add a padding component or just apply padding styles to your container component if applicable.
...
If your symbols contain relative links, clicking them when on the IDX broker page will cause them to look for a relative path on the IDX Broker site. This is because the IDX wrapper will be displayed in a domain different from your other pages (about, services, home, etc.). Click here to [learn more about absolute and relative URLs](https://www.seoclarity.net/resources/knowledgebase/difference-relative-absolute-url-15325/).
...
It's quite a pain, especially if you have many links, but as far as we know, it's the only way.
...
After you've finished that, pass through it one more time and make sure you don't have any more relative links. (don't forget the logo link back to the homepage!)
...
When the Legacy Widgets page is open, click the "Create New" button in the upper right corner.
...
We can actually override the default styles by using the custom CSS field when we create the widget.

If you've already exited the widget creation window or need to return to the CSS part, head back to the Legacy Widgets page (Design > Widgets > Legacy), click the Edit Widget button next to the widget name, and scroll down to the "Modify widget styling" section.
...
I've created a starter file to help you modify the element's CSS. The file overrides the default values and provides a bunch of empty selectors for you to style the widget as you like.
...
The Showcase widget is a simple way to showcase your listings on any website page. It displays as a grid and links to more information about each listing.

##### Creating the Showcase widget:

Creating the showcase widget is similar to creating the Quick Search Widget. In the left-hand menu, go to Design > Widgets > Legacy, click on the gray "Create Widget" button in the upper right, and select Showcase from the dropdown menu.

Now, you can choose which listings to display in the widget. Here are some of the most common or most selected options.

*   **Featured Properties:** These are properties that are listed by you (agent plan) or your brokerage (Office and Team plans)
...
*   **Custom search:** allows you to create a custom search, such as for a specific city, ZIP code, price point, or other criteria.
...
Just like the quick search widget, you can view its structure by right-clicking and inspecting with Chrome DevTools or using the (simplified) structure below.
...
Head back to the Legacy Widgets page (Design > Widgets > Legacy), click the Edit Widget button next to your newly created Showcase widget, and scroll down to the "Modify widget styling" section.
...
Just like the previous widget, I've created a template style guide for you to use that eliminates most (if not all) of the default styling on the Showcase widget and sets you off with a cleaner, more modern design. See below:
...
Sometimes, you or your client may want to integrate Webflow's CMS collection pages with an IDX Broker widget to dynamically widgets based on the current collection page.

If I have a brokerage based in Los Angeles, California, I may serve the surrounding areas, such as Beverly Hills, Pasadena, and Long Beach. Having a page for each of these cities might benefit SEO for anyone searching "houses for sale in Beverly Hills" or "homes for sale in Pasadena."

On each collection page, I want to display a Showcase widget with the city filter set to the current collection page so my Los Angeles collection page shows results from Los Angeles. How do I do this?

#### Create widgets for each of your collection items

First, you'll need to create a new widget in IDX Broker for each of your collection items. Luckily, the number of widgets you can have on IDX Broker isn't limited.
...
Next, we duplicated the widget in the Legacy Widgets view using the duplicate button next to the widget's name. We changed the widget's name to the next city and changed only the city property to match the name.
...
We're going to use each widget's unique ID to change which widget is shown on which page dynamically.

In your CMS Collection Settings, you can add a number field for the widget's ID. Ours is called "IDX Number," and that's what we'll call it for the rest of the steps.
...
In each collection item, change their IDX Number property to the corresponding widget's ID in IDX Broker.
...
Awesome! Your widgets have been successfully linked to your Webflow CMS. Now, we just have to add them to the collection page.
...
In Webflow, head to your Collection Page template and drop in an HTML Embed element. Paste it into your widget's code, and you should have something that looks like this.
...
Make sure everything looks good, save and close, and publish your site. If you followed everything correctly, when you open up your collection page on the live site, you should see the custom widget you created.

### 7\. Submit to local MLS for final approval

Congratulations, you've successfully implemented IDX and Webflow!

Before your site can go live, ask your local MLS what specifications it needs to get final MLS approval.

Every MLS is different, so contact your local MLS and ask what specifications are needed. They can also help you get your site in compliance and learn how to submit it for final approval.
